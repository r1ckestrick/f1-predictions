...
def save_predictions():
    ...
    try:
        ...
        # ✅ Cálculo de puntos (usando resultados simulados por ahora)
        try:
            results_response = requests.get(f"https://api.jolpi.ca/ergast/f1/{season}/{race}/results.json")
            if results_response.status_code == 200:
                ...
                parsed_results = {
                    "pole": results[0]["Driver"]["code"],
                    "p1": results[0]["Driver"]["code"],
                    "p2": results[1]["Driver"]["code"],
                    "p3": results[2]["Driver"]["code"],
                    "positions_gained": results[2]["Driver"]["code"],
                    "positions_lost": results[2]["Driver"]["code"],
                    "fastest_lap": results[0]["Driver"]["code"],
                    "dnf": None,
                    "best_of_the_rest": None,
                    "midfield_master": None
                }

                prediction.points = calculate_points(prediction, parsed_results)
                print("✅ Puntos calculados:", prediction.points)
            else:
                print("⚠️ No se pudieron obtener resultados reales")
        except Exception as err:
            print("⚠️ Error al calcular puntos:", err)

        db.session.add(prediction)  # Por si acaso
        db.session.commit()
...
def get_race_results(season, round):
    ...
    if response.status_code == 200:
        ...
        return jsonify({
            "winner": results[0]["Driver"]["code"] if results else "N/A",
            "pole_position": results[0]["Driver"]["code"] if results else "N/A",
            # ⚠️ Reemplaza con datos reales cuando tengas más lógica para fastest lap, etc.
        })
